...
llvm::Value* CodeGenCPU::CreateCallExtern(const CallNode* op) {
  ...
  llvm::FunctionType* ftype = llvm::FunctionType::get(LLVMType(op->dtype), arg_types, false);
...
llvm::Value* CodeGenCPU::GetPackedFuncHandle(const std::string& fname) {
  ...
  ctx->setMetadata("tbaa",
                   md_builder_->createTBAAStructTagNode(md_tbaa_ctx_ptr_, md_tbaa_ctx_ptr_, 0));
  llvm::Value* retcode =
      builder_->CreateCall(RuntimeTVMGetFuncFromEnv(), {ctx, GetConstString(fname), out});
...
llvm::BasicBlock* CodeGenCPU::MakeCallPacked(const Array<PrimExpr>& args, llvm::Value** rvalue,
  ...
  *rvalue = builder_->CreateAlignedLoad(
      builder_->CreatePointerCast(ret_value, LLVMType(r_api_type)->getPointerTo()), 8);

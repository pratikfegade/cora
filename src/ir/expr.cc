...
PrimExpr::PrimExpr(std::string str) : PrimExpr(tir::StringImmNode::make(str)) {}
...
const PrimExpr RangeNode::max_inclusive() const { return min + extent - 1; }
const PrimExpr RangeNode::max_exclusive() const { return min + extent; }

Range Range::make_by_min_extent(PrimExpr min, PrimExpr extent) {
  return Range(make_object<RangeNode>(min, extent));
}

Range Range::make_by_min_max_inclusive(PrimExpr min, PrimExpr max_inclusive) {
  return Range(make_object<RangeNode>(min, max_inclusive - min + 1));
}

Range Range::make_by_min_max_exclusive(PrimExpr min, PrimExpr max_exclusive) {
  return Range(make_object<RangeNode>(min, max_exclusive - min));
}

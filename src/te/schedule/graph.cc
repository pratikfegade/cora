...
FeedGraph CreateFeedGraph(const ReadGraph& g) {
  ...
  for (auto kv : g) {
    for (Tensor t : kv.second) {
      // std::cout << "[CFG]   Edge: " << t << " " << kv.first << std::endl;
...
void PrintFeedGraph(const FeedGraph& g, std::string prefix) {
  for (const auto& p : g) {
    std::cout << prefix << " T: " << p.first << " " << p.first->op << std::endl;
    for (const auto& op : p.second) {
      std::cout << prefix << "   Op: " << op << std::endl;
    }
  }
}
    ...
    if (const auto* scan_op = op.as<ScanOpNode>()) {
      ...
      for (size_t i = 0; i < update.size(); ++i) {
        ...
        for (int k = 0; k < static_cast<int>(update[i]->shape.size()); ++k) {
          ...
          if (!init[i]->op.as<PlaceholderOpNode>()) {

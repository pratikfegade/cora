 ...
 public:
  explicit BufferAnalyser(Map<Tensor, Buffer> extern_buffer,
    ...
    for (auto kv : extern_buffer) {
      ...
      bi.shape = kv.second->shape->get_dense_shape();
      bi.external = true;
      buf_map_[TensorKey{kv.first->op, kv.first->value_index}] = bi;
    }
  }
 ...
 public:
  ...
  Stmt VisitStmt_(const ProvideNode* op) final {
    ...
    if (it != mma_sync_.end()) {
      ...
      auto mma_sync_call = [&buffer_node_a, &buffer_node_b](const Buffer& buffer) {
...
Stmt RewriteForTensorCore(Stmt stmt, Schedule schedule, Map<Tensor, Buffer> extern_buffer) {

    ...
    if (op->attr_key == attr::thread_extent) {
      ...
      if (iv->thread_tag == "threadIdx.x") {
        ...
        CHECK(arith::GetConstInt(op->value, &value) && value == warp_size_)
            << "Expect threadIdx.x 's size to be equal to warp size(" << warp_size_ << ")"
 ...
 protected:
  ...
  Stmt VisitStmt_(const StoreNode* op) {
    if (op->buffer_var.get() == buffer_) {
      ...
      return StoreNode::make(op->buffer_var, op->value, local_index, op->predicate, op->sync_type);
    } else {
  ...
  PrimExpr Mutate_(const LoadNode* op) {
    if (op->buffer_var.get() == buffer_) {
      ...
      PrimExpr load_value =
          LoadNode::make(op->dtype, op->buffer_var, local_index, op->predicate, op->sync_type);
      return CallNode::make(load_value.dtype(), intrinsic::tvm_warp_shuffle, {load_value, group},
                            CallNode::Intrinsic);
    } else {
      return StmtExprMutator::VisitExpr_(op);
    }
  ...
  std::pair<PrimExpr, PrimExpr> SplitIndexByGroup(const PrimExpr& index) {
    ...
    if (warp_group_ == 1) {
      ...
      y = y * m + x;
      PrimExpr z =
          indexdiv(indexmod(index, make_const(index.dtype(), warp_coeff_ * warp_size_)), m);

...
RewriteSimplifier::Impl::CompareResult RewriteSimplifier::Impl::TryCompare(const PrimExpr& x,
  ...
  // std::cout << "[TryCompare] diff " << diff << std::endl;
  ...
  // std::cout << "[TryCompare] Bound " << dbound->min_value << " " << dbound->max_value <<
  // std::endl;
...
void RewriteSimplifier::Impl::Update(const Var& var, const PrimExpr& info, bool override) {
  if (!override) {
...
PrimExpr RewriteSimplifier::Impl::VisitExpr_(const FloorDivNode* op) {
  ...
  if (IsIndexType(op->dtype)) {
    ...
    // TVM_TRY_REWRITE_IF(floordiv(x * c1 + y, c2), floordiv(x, c2.Eval()->value /
    // c1.Eval()->value), c1.Eval()->value > 0 && c2.Eval()->value > 0 && c2.Eval()->value %
    // c1.Eval()->value == 0 && TryCompare(y.Eval(), c2.Eval()->value) == kLT);
...
PrimExpr RewriteSimplifier::Impl::VisitExpr_(const LTNode* op) {
  ...
  if (IsIndexType(op->a.dtype())) {
    CompareResult result = TryCompare(op->a - op->b, 0);
    if (result == kLT) {
      // std::cout << "[RSLT2] " << GetRef<PrimExpr>(op) << " " << ret << std::endl;
      return make_const(op->dtype, true);
    }
    if (result == kEQ || result == kGT || result == kGE) {
      // std::cout << "[RSLT3] " << GetRef<PrimExpr>(op) << " " << ret << std::endl;
      return make_const(op->dtype, false);
    }
    ...
    TVM_TRY_REWRITE_IF(truncdiv(x, c1) * c1<x, 0 < truncmod(x, c1), c1.Eval()->value> 0);
    TVM_TRY_REWRITE_IF(truncdiv(x, c1) * c1<x + y, 0 < truncmod(x, c1) + y, c1.Eval()->value> 0);
    TVM_TRY_REWRITE_IF(truncdiv(x, c1) * c1 < x - y, y<truncmod(x, c1), c1.Eval()->value> 0);

    TVM_TRY_REWRITE_IF(truncdiv(x + c2, c1) * c1 < x, c2<truncmod(x + c2, c1), c1.Eval()->value> 0);
    ...
    TVM_TRY_REWRITE_IF(floordiv(x + c2, c1) * c1 < x, c2<floormod(x + c2, c1), c1.Eval()->value> 0);
  ...
  // std::cout << "[RSLT] " << GetRef<PrimExpr>(op) << " " << ret << std::endl;
  return ret;
}
...
PrimExpr RewriteSimplifier::operator()(const PrimExpr& expr) {
  ...
  for (int i = 0; i < max_iter; ++i) {
    ...
    res = new_expr;
    // std::cout << "  Operator() Iter " << i << " " << res << std::endl;
  }
  return res;
}

...
class PSelectExpr : public Pattern<PSelectExpr<TCond, TA, TB>> {
 public:
  PSelectExpr(const TCond& condition, const TA& true_value, const TB& false_value,
              const PVar<tir::FunctionRef>& fi_fun, const PVar<PrimExpr>& fi_expr)
  ...
  bool Match_(const ObjectRef& node) const {
    // if (const tir::FuseSelectNode* ptr = node.as<tir::FuseSelectNode>()) {
    // return false;
    // } else
    if (const tir::SelectNode* ptr = node.as<tir::SelectNode>()) {
      ...
      // std::cout << "Matching " << id << " " << ptr->condition << std::endl;
      if (const tir::FuseSelectNode* ptr = node.as<tir::FuseSelectNode>()) {
        fi_fun_.Match_(ptr->fi_fun);
        fi_expr_.Match_(ptr->fused_val);
        // std::cout << "  FuseSelect " << id << " " << (fi_fun_.filled() && fi_expr_.filled())
        // << std::endl;
      }
      return true;
    } else {
  ...
  PrimExpr Eval() const {
    // std::cout << " Eval FuseSelect "
    // << " " << id << " " << (fi_fun_.filled() && fi_expr_.filled()) << std::endl;
    if (fi_fun_.filled() && fi_expr_.filled()) {
      return tir::FuseSelectNode::make(condition_.Eval(), true_value_.Eval(), false_value_.Eval(),
                                       fi_fun_.Eval(), fi_expr_.Eval());
    } else {
 ...
 private:
  ...
  PVar<tir::FunctionRef>::Nested fi_fun_;
  PVar<PrimExpr>::Nested fi_expr_;
  int id;

  // mutable bool is_fuse_select = false;
  // mutable tir::FunctionRef fi_fun;
  // mutable PrimExpr fused_val;
};
...
inline PSelectExpr<TCond, TA, TB> select(const Pattern<TCond>& condition,
                                         ...
                                         const PVar<tir::FunctionRef>& fi_fun,
                                         const PVar<PrimExpr>& fi_expr) {
  return PSelectExpr<TCond, TA, TB>(condition.derived(), true_value.derived(),
                                    false_value.derived(), fi_fun, fi_expr);
}
